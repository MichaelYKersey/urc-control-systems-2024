...
void application()
{
  ...
  can_perseus pitch_can(allowed_id_pitch,
                        baudrate,
                        can_transceiver,
                        can_bus_manager,
                        can_interrupt,
                        can_id_filter);
  auto pitch_can_ptr = hal::v5::make_strong_ptr<decltype(pitch_can)>(
    resources::driver_allocator(), std::move(pitch_can));
  can_perseus roll_can(allowed_id_roll,
                       baudrate,
                       can_transceiver,
                       can_bus_manager,
                       can_interrupt,
                       can_id_filter);
  auto roll_can_ptr = hal::v5::make_strong_ptr<decltype(pitch_can)>(
    resources::driver_allocator(), std::move(roll_can));
  ...
  // bool neg = 1; // 0 = pos, 1 = neg (this only matters for pitch (left should
  // be negative))
  int delay_counter = 0;
  ...
  while (true) {
    ...
    // receive message
    std::optional<hal::can_message> msg_p =
      pitch_can_ptr->check_for_mc_message();
    std::optional<hal::can_message> msg_r =
      roll_can_ptr->check_for_mc_message();
